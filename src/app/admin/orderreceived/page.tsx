      ...
      onConfirm: async () => {
        try {
          ...
          if (data.success) {
            ...
            showAlert({ type: "success", message: "Order marked as delivered." });
          } else {
            showAlert({ type: "error", message: data.error || "Failed to update order" });
          }
        } catch (error) {
          console.error("Failed to update order", error);
          showAlert({ type: "error", message: "An error occurred while updating the order." });
        }
      }
    });
  };

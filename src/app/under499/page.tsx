...
function Under499Content() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/user/under499?sort=${encodeURIComponent(currentSort)}&page=${currentPage}&limit=25`);
  ...
  const fetchSingleProduct = useCallback(async () => {
    ...
    setError("");
    try {
      // We need the collection name to fetch a single product details
  ...
  const handleSortChange = (newSort: string) => {
    ...
    params.set('page', '1');
    router.push(`/under499?${params.toString()}`);
  };
    ...
    <div className="pt-4 md:pt-8 pb-24 px-4 md:px-8 max-w-7xl mx-auto min-h-screen">
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-2 md:space-y-4">
          <p className="text-[9px] md:text-[10px] font-black tracking-[0.4em] text-rose-500 uppercase">
            Value Selection
          </p>
          <h1 className="text-4xl md:text-7xl font-black text-black tracking-tighter uppercase leading-none">
            Under <span className="text-rose-600">₹499</span>
          </h1>
          <div className="h-1 w-12 md:w-20 bg-rose-600 mt-4 md:mt-6"></div>
        </div>
        ...
        <div className="flex flex-col gap-2 min-w-[200px]">
          ...
          <div className="relative group">
            <select 
              ...
              className="w-full bg-white border-2 border-zinc-100 rounded-xl px-4 py-3 text-xs font-bold uppercase tracking-widest appearance-none outline-none focus:border-rose-600 transition-all cursor-pointer"
            >
      ...
      {loading ? (
        <div className="min-h-[40vh] flex flex-col items-center justify-center">
          <div className="w-12 h-12 border-4 border-zinc-200 border-t-rose-600 rounded-full animate-spin"></div>

...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <>
            {/* Overlay */}
            <div
                className={`fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm transition-opacity duration-500 lg:hidden ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"
                    }`}
                onClick={onClose}
            />
            ...
            {/* Sidebar Panel */}
            <aside
                className={`fixed inset-y-0 left-0 z-[70] w-72 transform border-r border-black bg-white text-black transition-transform duration-500 ease-in-out lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                        ...
                        <nav className="space-y-2">
                            {navItems.map((item) => (
                                <Link
                                    ...
                                    href={item.href}
                                    onClick={() => onClose()}
                                    className={`group flex items-center justify-between rounded-none px-4 py-4 text-xs font-black uppercase tracking-widest transition-all duration-200 border border-transparent ${isActive(item.href)
                                            ? "bg-black text-white"
                                ...
                                {user ? (
                                    <>
                                        ...
                                        <button
                                            ...
                                            className="w-full flex items-center gap-4 px-4 py-3 text-xs font-bold text-black/60 hover:text-black transition-colors"
                                        >
                                            <X className="h-4 w-4" /> Sign Out
                                        </button>
                                    </>
                                ) : (
                                    <Link href="/login" onClick={() => onClose()} className="flex items-center gap-4 px-4 py-3 text-xs font-bold text-black/60 hover:text-black transition-colors">

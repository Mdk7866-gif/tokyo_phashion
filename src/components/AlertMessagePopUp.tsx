...
const AlertMessagePopUp: React.FC<AlertMessagePopUpProps> = ({
  isOpen,
  onClose,
  title,
  message,
  ...
  return (
    ...
    >
      <div
        ...
        className={`
          relative w-full border-t-2 border-black bg-white p-6 shadow-[0px_-4px_10px_rgba(0,0,0,0.1)]
          transition-all duration-500 ease-out
          fixed bottom-0 left-0 right-0
          lg:relative lg:bottom-auto lg:left-auto lg:right-auto lg:max-w-sm lg:border-2 lg:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]
          ${isOpen ? "translate-y-0 opacity-100" : "translate-y-full opacity-0 lg:translate-y-4 lg:opacity-0"}
        `}
      >
        <div className="flex flex-col items-center text-center">
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-zinc-50 border border-zinc-100">
            {icons[type]}
          </div>

          <h3 className="mb-2 text-base font-black uppercase italic tracking-tight lg:text-lg">
            {title}
          </h3>
          ...
          <button
            onClick={onClose}
            className="w-full border border-black bg-black px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white transition-all hover:bg-zinc-800 active:translate-y-1 lg:py-3 lg:text-xs"
          >
            {buttonText}
          </button>
        </div>
